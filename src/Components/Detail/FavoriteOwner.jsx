          ...
          <Col lg={12} className="d-flex flex-column align-items-center">
            ...
            <Tab.Content>
              <Tab.Pane eventKey="All">
                <FavoriteOwner properties={properties} onRemove={handleRemoveProperty} setProperties={setProperties} />
              </Tab.Pane>
...
const FavoriteOwner = ({ properties, onRemove, setProperties }) => {
  ...
  useEffect(() => {
    ...
    const fetchOwnerFavoriteProperties = async () => {
      try {
        const response = await axios.get(`${process.env.REACT_APP_API_URL}/get-favorite-owner`, { params: { phoneNumber } });
        if (response.status === 200) {
          setProperties(response.data.favoriteRequestsData);
        } else {
