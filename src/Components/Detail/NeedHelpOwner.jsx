          ...
          <Col lg={12} className="d-flex flex-column align-items-center">
            ...
            <Tab.Content>
              <Tab.Pane eventKey="All">
                <NeedHelpOwner properties={properties} onRemove={handleRemoveProperty} setProperties={setProperties} />
              </Tab.Pane>
...
const NeedHelpOwner = ({ properties, onRemove, setProperties }) => {
  ...
  useEffect(() => {
    ...
    const fetchNeedHelpOwnerProperties = async () => {
      try {
        const response = await axios.get(`${process.env.REACT_APP_API_URL}/get-help-as-owner`, { params: { phoneNumber } });
        if (response.status === 200) {
          setProperties(response.data.helpRequestsData);
        } else {
