          ...
          <Col lg={12} className="d-flex flex-column align-items-center">
            ...
            <Tab.Content>
              <Tab.Pane eventKey="All">
                <ReportPropertyOwner properties={properties} onRemove={handleRemoveProperty} setProperties={setProperties} />
              </Tab.Pane>
...
const ReportPropertyOwner = ({ properties, onRemove, setProperties }) => {
  ...
  useEffect(() => {
    ...
    const fetchOwnerReportProperties = async () => {
      try {
        const response = await axios.get(`${process.env.REACT_APP_API_URL}/get-reportProperty-owner`, { params: { phoneNumber } });
        if (response.status === 200) {
          setProperties(response.data.reportPropertyRequestsData);
        } else {
