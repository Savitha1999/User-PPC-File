          ...
          <Col lg={12} className="d-flex flex-column align-items-center">
            ...
            <Tab.Content>
              <Tab.Pane eventKey="All">
                <ContactOwner properties={properties} onRemove={handleRemoveProperty} setProperties={setProperties} />
              </Tab.Pane>
...
const ContactOwner = ({ properties, onRemove, setProperties }) => {
  ...
  useEffect(() => {
    ...
    const fetchContactOwnerProperties = async () => {
      try {
        const response = await axios.get(`${process.env.REACT_APP_API_URL}/get-contact-owner`, { params: { phoneNumber } });
        if (response.status === 200) {
          setProperties(response.data.contactRequestsData);
        } else {
