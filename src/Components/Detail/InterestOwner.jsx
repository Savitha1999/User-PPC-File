          ...
          <Col lg={12} className="d-flex flex-column align-items-center">
            ...
            <Tab.Content>
              <Tab.Pane eventKey="All">
                <InterestOwner properties={properties} onRemove={handleRemoveProperty} setProperties={setProperties} />
              </Tab.Pane>
...
const InterestOwner = ({ properties, onRemove, setProperties }) => {
  ...
  useEffect(() => {
    ...
    const fetchInterestedProperties = async () => {
      try {
        const response = await axios.get(`${process.env.REACT_APP_API_URL}/get-interest-owner`, { params: { phoneNumber } });
        if (response.status === 200) {
          setProperties(response.data.interestRequestsData);
        } else {
...
<FaEye className="me-1" />1
   ...
   <div className="col-md-8 col-8 ps-2">
    <div className="d-flex justify-content-between"><p className="m-0" style={{ color:'#5E5E5E' , fontWeight:'normal' }}>{property.propertyMode || 'N/A'}</p>  {property.phoneNumber || 'N/A'}
...
<FaEye className="me-1" />1
                 ...
                 <div className="col-md-8 col-8 p-0">
                  <div className="d-flex justify-content-between"><p className="mb-1 fw-bold" style={{ color:'#5E5E5E' }}>{property.propertyMode || 'N/A'}</p> {property.phoneNumber || 'N/A'}
